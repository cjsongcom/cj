...
inline cjhash_val cjhash_mbcs_hasher(const void* _str) {
	const cjmc* str = _str;
	cjui hash = 0; cji c;
...
inline cjhash_val cjhash_ptr_hasher(const void* p) {
...
inline cjrst cjhash_mbcs_cmp(const void* lhs, const void* rhs) {
	return cjstrcmp((const cjmc*)lhs, (const cjmc*)rhs);
}

inline cjrst cjhash_ptr_cmp(const void* lhs, const void* rhs) {
...
inline void cjhash_remover_default(void* pair) {	
	CJ_UNUSED(pair)
}
...
CJEXTERNC cjhash_val cjhash_hash(cjhash* hash, void* key);
...
// key=void*, val=void*
CJEXTERNC cjhash_pair* cjhash_push(cjhash* hash, void* ptr_key, void* ptr_val);
